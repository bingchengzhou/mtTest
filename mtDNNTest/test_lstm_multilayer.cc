...
void load_weight_bias_and_check(std::vector<std::vector<float>> &weight_vecs,
                                ...
                                bool is_hidden = false, int layer_num = 0) {
  ...
  wh_file_path +=
      (is_hidden ? "lstm_multilayer/lstm_weight_hh_l" : "lstm_multilayer/lstm_weight_ih_l") +
      std::to_string(layer_num) + (is_reverse ? "_reverse.txt" : ".txt");
  bh_file_path += (is_hidden ? "lstm_multilayer/lstm_bias_hh_l" : "lstm_multilayer/lstm_bias_ih_l") +
...
int main() {
  using ::mt::dnn::Tensor;
  std::string input_file_path(DATA_DIR "lstm_multilayer/lstm_input.txt");
